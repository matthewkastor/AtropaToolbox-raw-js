...
atropa.CookieMonster = function CookieMonster() {
    ...
    function init() {
        function unsupported(e) {
            atropa.data.CookieMonster.error = 'The atropa.CookieMonster ' + 'class requires that document.cookie exists. ' + e;
...
atropa.CreateHtmlDocumentsFromXmlhttp = function CreateHtmlDocumentsFromXmlhttp() {
    ...
    function init() {
        ...
        try {
            ...
            atropa.data.CreateHtmlDocumentsFromXmlhttp.error = 'The atropa.CreateHtmlDocumentsFromXmlhttp ' + 'class requires the atropa.HTMLParser and the ' + 'atropa.Requester one of which has thrown the following ' + 'error: ' + e;
            throw new Error(atropa.data.CreateHtmlDocumentsFromXmlhttp.error);
        }
    }
    init();
        ...
        try {
            ...
            try {
                if (my.doc.nodeType !== 9) {
                    throw new Error('the document nodeType returned an ' + 'unexpected value');
                }
            } catch (e) {
                throw new Error('atropa.HTMLParser can not create a new ' + 'document because: ' + e);
            }
            try {
                my.loadString('<head></head><body><p id="testPara">test</p></body>');
            } catch (f) {
                throw new Error('atropa.HTMLParser can not load ' + 'the hidden document from string because: ' + f);
            }
            try {
                if (my.doc.getElementById('testPara').textContent !== 'test') {
                    throw new Error('the test textContent was not the ' + 'expected value');
                }
            } catch (g) {
                throw new Error('atropa.HTMLParser can not access ' + 'or manipulate the hidden document because: ' + g);
            }
        } catch (h) {
            atropa.data.HTMLParser.support = 'unsupported';
            atropa.data.HTMLParser.error = 'The atropa.HTMLParser Class can ' + 'not be used, it is not supported by the current environment ' + 'because: ' + h;
...
atropa.inject.element = function (elementType, docref, parentNod, attributes, onloadHandler, callback) {
    ...
    onloadHandler = atropa.setAsOptionalArg(function () {
    }, onloadHandler);
    callback = atropa.setAsOptionalArg(function () {
    }, callback);
...
atropa.inject.script = function (id, srcURL, docref, callback) {
    'use strict';
    atropa.supportCheck('inject');
    var attributes, elementType, parentNod, onloadHandler, el;
    attributes = {
        'id': id,
...
(function () {
    ...
    atropa.requires('Requester', function () {
        ...
        return supported;
    }, 'atropa.Requester requires ' + prerequisites + ' in order to be useful. This class is not supported in ' + 'this environment');
}());
...
(function () {
    ...
    atropa.requires('TextAnalyzer', function () {
        ...
        return supported;
    }, 'atropa.TextAnalyzer requires ' + prerequisites + ' in order to be useful. This class is not supported in ' + 'this environment');
}());
...
atropa.waitFor.test = function test(testFn, onSuccessCallback, onMaxPollCallback, pollInterval, maxPoll) {
    ...
    onMaxPollCallback = atropa.setAsOptionalArg(function () {
    }, onMaxPollCallback);
    onSuccessCallback = atropa.setAsOptionalArg(function () {
    }, onSuccessCallback);
    var myInt, myCounter;
    myCounter = 0;
...
atropa.requires('window', function () {
    'use strict';
    if (document === undefined) {
        return false;
    }
    return true;
}, 'atropa.window requires the window object present in web ' + 'browsers in order to be useful. This function is not supported in ' + 'this environment');
atropa.window = {};
...
(function () {
    ...
    atropa.requires('wtf', function () {
        ...
        return supported;
    }, 'atropa.wtf requires ' + prerequisites + ' in order to be useful. This class is not supported in ' + 'this environment');
}());
(function () {
    'use strict';
    var prerequisites = [window];
    atropa.requires('wtfHtmlElement', function () {
        ...
        return supported;
    }, 'atropa.wtf.htmlElement requires ' + prerequisites + ' in order to be useful. ' + 'This class is not supported in this environment');
}());
atropa.wtf = {};
